...
class BluetoothOperation(
    ...
    fun printQRCode(content: String, moduleSize: Int, height: Int, position: Int): Boolean {
        return try {
            printerInstance?.let {
                it.init()
    ...
    fun printBarcode(content: String, type: Int, width: Int, height: Int, position: Int): Boolean {
        return try {
            printerInstance?.let {
                it.init()
    ...
    fun printImage(base64Image: String): Boolean {
        return try {
            printerInstance?.let {
                it.init()
    ...
    fun setTextAlignment(alignment: Int): Boolean {
        return try {
            printerInstance?.let { printer ->
                ...
                // Log the alignment attempt
                android.util.Log.d("PrinterPlugin", "Setting alignment: $alignment")

                // Use the correct method with two parameters
                ...
                // Log the result
                android.util.Log.d("PrinterPlugin", "Alignment set result: $result")
    ...
    fun setEncoding(encoding: String): Boolean {
        return try {
            printerInstance?.let {
                it.setEncoding(encoding)  // Use method instead of property assignment
                true
            } ?: false
        } catch (e: Exception) {
            e.printStackTrace()
